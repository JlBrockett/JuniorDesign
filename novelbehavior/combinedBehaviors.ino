...
/*
The starting position is the same as the bot two in challenge 1.
Recallibrate delays to achieve desired turn angles.
Sequence:
1. Move forward while no collision
2. If collison, stop and blow horn, then turn 180 degress
4. Stop and move forward for some time (tweak delay time to keep bot in bounds)
5. Stop and turn right.
6. POST message and GET the same message immediately. 
7. Play nokia song when message is received
8. Move forward again to detect collision.
9. After collision, stop tuen 180 and stop angain
10. Play Rick song to indicate end of sequence.
*/
...
void loop() {
  //Diagnostic Stuff. Can be taken out
  ...
  //clear lcd and move forward until collision is detected
  lcd.clear();
  detectCollision(); //check collision callibration to change treshold
  while (!wallOne) {
    forwardDirection();
    detectCollision();
  }
  ...
  stopMotor();
  //play horn function to demosntrate novel behaviour
  horn();
  delay(500);
  //180 turn. recallibrate delay time to properly achieve this
  turnRight();
  delay(1000);
  ...
  //move forward, top turn right, stop and prepare to post and get server messages 
  forwardDirection();
  delay(2000);
  stopMotor();

  delay(800);
  turnRight(); //90 degress turn
  delay(8000);
  stopMotor();
  delay(800);

  //LED stuff. Can be taken out
  digitalWrite(redLed, HIGH);
  digitalWrite(blueLed, HIGH);
  horn();
  delay(250);
  ...
  //post message  and wait to get
  POSTServer(postRoute, postBody);
  GetServer(getRoute, messageData);
  if (client.available()) { // If message is available,
    //play nokia tone to show novel behaviour. Check the code for this. It takes a little too long to finish playing
    nokia();
    //read bytes and do nothing with it.
    while(client.available()) {
      client.read();
    }
  }

  //move forward to detect second wall
  detectCollision();
  while (!wallTwo) {
    forwardDirection();
    detectCollision();
  ...
  //180 degree turn. recallibrate to achieve this
  turnRight();
  delay(1500);
  stopMotor();

  //play rick song to indicate end of loop
  rick();
...
void nokia() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzer, nmelody[thisNote], nnoteDuration * 0.9);
