...
void loop() {
  ...
  while (true) {
    ...
    if (client.available()) { // If message is available,
      ...
      break;  // Exit loop to start main loop
    }
    delay(5000);
  }
  ...
  //follow blue line until collision is detected
  detectCollision();
  digitalWrite(blueLed, HIGH);
  while (!wallTwo) {
    redLineFollowing();
  ...
  //follow yellow line until collision
  while (!wallThree) {
    redLineFollowing();
    detectCollision();
  }
  ...
  //play rick song to end challenge
  rick();
